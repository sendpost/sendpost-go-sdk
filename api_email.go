...
func (a *EmailAPIService) SendEmailExecute(r ApiSendEmailRequest) ([]EmailResponse, *http.Response, error) {
	...
	if r.emailMessageObject == nil {
		return localVarReturnValue, nil, reportError("emailMessageObject is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.emailMessageObject
	if r.ctx != nil {
...
type ApiSendEmailWithTemplateRequest struct {
	ctx context.Context
	ApiService *EmailAPIService
	emailMessageWithTemplate *EmailMessageWithTemplate
}
...
func (r ApiSendEmailWithTemplateRequest) EmailMessageWithTemplate(emailMessageWithTemplate EmailMessageWithTemplate) ApiSendEmailWithTemplateRequest {
	r.emailMessageWithTemplate = &emailMessageWithTemplate
	return r
}
...
func (a *EmailAPIService) SendEmailWithTemplateExecute(r ApiSendEmailWithTemplateRequest) ([]EmailResponse, *http.Response, error) {
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.emailMessageWithTemplate == nil {
		return localVarReturnValue, nil, reportError("emailMessageWithTemplate is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.emailMessageWithTemplate
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["subAccountAuth"]; ok {
